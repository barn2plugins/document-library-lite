...
class Upsell extends Step {
	...
	public function __construct() {
		$this->set_id( 'more' );
		$this->set_name( esc_html__( 'More', 'document-library-for-wordpress' ) );
		$this->set_description(
			sprintf(
				// translators: %1$s: URL to All Access Pass page %2$s: URL to the KB about the upgrading process
				__( 'Enhance your store with these fantastic plugins from Barn2, or get them all by upgrading to an <a href="%1$s" target="_blank">All Access Pass<a/>! <a href="%2$s" target="_blank">(learn how here)</a>', 'document-library-for-wordpress' ),
		...
		$this->set_title( esc_html__( 'Extra features', 'document-library-for-wordpress' ) );
	}
