...
class Completed extends Ready {
	...
	public function __construct() {
		parent::__construct();
		$this->set_name( esc_html__( 'Ready', 'document-library-for-wordpress' ) );
		$this->set_title( esc_html__( 'Complete Setup', 'document-library-for-wordpress' ) );
	...
	private function get_custom_description() {
		...
		return sprintf(
			...
			esc_html__( 'Congratulations, you have finished setting up the plugin! The next step is to start %1$sadding%2$s documents. Your documents will be listed on the %3$sdocument library page%4$s.', 'document-library-for-wordpress' ),
